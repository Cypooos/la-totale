...
*Petites questions*
- Soit un jeu $G=(V,E)$ avec $V = NN$ et tel que les arêtes orientées sont toutes de la forme $(x,y)$ avec $y<x$. On suppose de plus que $forall x in V, deg_+(x) = 0 => x in F_1 union F_2$. Montrer que pour chaque état du jeu, un des deux joueurs possède toujours une stratégie gagnante.
...
3. Donner l’ensemble des états finaux sur le graphe $P_5$, le chemin de longueur 4, de sommets $[|0;4|]$. On suppose que R commence par colorier le sommet 1.
...
Soit $A subset.eq NN^*$. On considère un jeu à deux joueurs où $N>0$ objets sont disposés sur une table, et chaque joueur doit à tour de rôle retirer $t in A$ objets de la table (s'il peut). Le joueur qui retire le dernier objet perd, et un joueur qui ne peut pas jouer perd.
...
3. Si $A$ est fini et $max(A) <= N$, proposer un algorithme qui décide si Alice possède une stratégie gagnante pour un $N$ donné.
...
On supposera maintenant sans perte de généralité que le premier coup d'Alice est $a...a$. On pose $G_n = (V_n,E_n)$ le graphe définit par $V_n = Sigma^n$ et $(u,v) in E_n$ si $u$ et $v$ ne diffèrent que d'une lettre.

3. Montrer par récurrence sur $n in NN^*$ que $G_n - w_a$ possède un couplage parfait
4. En déduire que Alice possède une stratégie gagnante.
5. Déterminer le gagnant pour $Sigma = {a,b,c,d}$, puis généraliser pour tout alphabet fini non vide $Sigma$.

== Géographie#footnote[Marathon de math de Paris-Orsay 2021]

Soit $G = (V,E)$ un graphe, on définit le jeu à deux joueurs (A)lice et (B)ob tel que à tour de role, chaque joueur doit marquer un sommet non déjà marqué qui soit un voisin du dernier sommet marqué. Le premier joueur qui ne peut plus marquer de sommet perd. Le premier joueur (Alice) peut choisir où commencer.

1. On considère $C_n$ le cycle de longueur $n$. Pour quel $n in NN$ est-ce que Alice a une stratégie gagnante sur $C_n$~? 
2. Montrer que si le graphe possède un couplage parfait, alors Bob possède une stratégie gagnante. La réciproque est-elle vraie~?
3. Proposer un algorithme dans le cas ou $G$ est un arbre pour savoir qui possède une stratégie gagnante.
4. On considère le cas ou $V subset.eq NN^2$ et $forall ((x,y),(x',y')) in E, |x-x'|+|y-y'| = 1$ avec $G$ connexe. Montrer que si $|V|$ est impair, alors Alice possède une stratégie gagnante. La réciproque est-elle vraie~? 
...
5. Proposer un algorithme qui décide, pour un entier $n$ et un automate fini déterministe complet $cal(A)$ si Alice possède une stratégie gagnante pour l'objectif des entiers associés aux mots de $L(cal(A))$. Quelle est sa complexité?
6. Montrer que l'on peut faire la question 5 en complexité $O(k 2^k + log n)$ pour $k$ le nombre d'états de l'automate. 
...
*Question 2* On suppose qu'il existe une partition $cal(P)$ de $V$ telle que $||cal(P)|| < 2(|cal(P)|-1)$. Montrer que Alice possède une stratégie gagnante.
...
*Théorème* On admet le théorème de Nash-Williams-Tutte: $G$ possède $k$ arbres couvrants deux à deux arête-disjoints si et seulement si pour toute partition $cal(P)$ de $V$, $||cal(P)|| >= k(|cal(P)|-1)$

*Question 4* Montrer que Bob a une stratégie gagnante si et seulement si $G$ possède deux arbres couvrants arête-disjoints.
